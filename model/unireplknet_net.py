...
class DilatedReparamBlockk(nn.Module):
    """
    Dilated Reparam Block proposed in UniRepLKNet (https://github.com/AILab-CVC/UniRepLKNet)
    We assume the inputs to this block are (N, C, H, W)
    """
    def __init__(self, in_channels, out_channels, deploy, kernel_size=9, use_sync_bn=True):
        super().__init__()
        self.lk_origin = get_conv2d(in_channels, out_channels, kernel_size, stride=1,
                                    padding=kernel_size//2, dilation=1, groups=1, bias=deploy,
                                    )
        ...
        for i in range(4):
            main_stage = nn.Sequential(
                *[UniRepLKNetBlock(dim=dims[i], kernel_size=kernel_sizes[i][j], 
                                   ...
                                   use_sync_bn=use_sync_bn) for j in
                  range(depths[i])])
