...
class Converter:
    def __init__(self, variant: str, checkpoint: str, device: str):
        self.model = MattingNetwork(variant,deploy=True).eval().to(device)
        self.model.load_state_dict(torch.load(checkpoint, map_location=device), strict=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--variant', type=str, required=True, choices=['unireplknet','mobilenetv3', 'resnet50'])
